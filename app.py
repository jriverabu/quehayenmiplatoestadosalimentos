...
# Añadir exactamente después de la última línea de la función detect_dates_with_gemini que termina con:
# return []

def detect_cooking_state(img, image_path):
    """
    Analiza el estado de cocción de los alimentos usando el modelo Gemini Pro Vision API.
    ...
    Returns:
        Un diccionario con el resultado del análisis de cocción, o None si hay un error
    """
    try:
        # Importar PIL.Image
        from PIL import Image
        ...
        # Crear mensaje para Gemini enfocado en el estado de cocción
        cooking_state_msg = ChatMessage(
            role=MessageRole.USER,
            blocks=[
                TextBlock(text="""Analiza esta imagen de alimento y determina su estado de cocción.
                
                1. Identifica el tipo de alimento presente en la imagen
                2. Determina el nivel de cocción (crudo, poco hecho, término medio, bien hecho, muy hecho, quemado)
                3. Evalúa si el alimento está en un punto óptimo de cocción
                4. Estima el tiempo de cocción adicional necesario (si aplica)
                
                Responde SOLO con un objeto JSON con el siguiente formato (sin texto adicional):
                {
                  "alimento": "nombre_del_alimento",
                  "estado_coccion": "nivel_de_coccion",
                  "punto_optimo": true/false,
                  "tiempo_adicional": "tiempo_estimado_en_minutos",
                  "recomendaciones": "sugerencias_específicas_para_mejorar_coccion",
                  "color": "descripción_del_color",
                  "textura": "descripción_de_la_textura",
                  "temperatura_recomendada": "temperatura_ideal_en_celsius_o_descripción",
                  "confianza": valor_entre_0_y_1
                }"""),
        ...
        # Obtener respuesta de Gemini
        cooking_response = gemini_pro.chat(messages=[cooking_state_msg])
        
        # Procesar la respuesta
        response_text = cooking_response.message.content
        
        # Extraer el JSON
        json_match = re.search(r'(\{.*\})', response_text, re.DOTALL)
        if json_match:
            json_str = json_match.group(1)
            cooking_result = json.loads(json_str)
            return cooking_result
        else:
            return None
            
    except Exception as e:
        print(f"Error al analizar el estado de cocción: {str(e)}")
        return None
        ...
        with analysis_tabs[2]:
            ...
            with st.spinner("Analizando estado del alimento con IA..."):
                ...
                try:
                    ...
                    # Añadir sección para punto de cocción después de las secciones existentes
                    st.markdown("<hr>", unsafe_allow_html=True)
                    st.subheader("Punto de Cocción")
                    
                    with st.spinner("Analizando punto de cocción del alimento..."):
                        st.info("Procesando imagen para determinar el estado de cocción...")
                        
                        # Obtener información sobre el punto de cocción
                        cooking_state = detect_cooking_state(img, temp_filename)
                        
                        if cooking_state:
                            # Diseño limpio y estructurado para mostrar el estado de cocción
                            st.success(f"Se ha analizado el punto de cocción de {cooking_state['alimento']}")
                            
                            # Determinar el color según el estado
                            if cooking_state['punto_optimo']:
                                cooking_color = "#4CAF50"  # Verde
                                cooking_bg = "#E8F5E9"
                                cooking_status = "Punto óptimo"
                            else:
                                if cooking_state['estado_coccion'].lower() in ['crudo', 'poco hecho']:
                                    cooking_color = "#F44336"  # Rojo
                                    cooking_bg = "#FFEBEE"
                                    cooking_status = "Necesita más cocción"
                                elif cooking_state['estado_coccion'].lower() in ['muy hecho', 'quemado']:
                                    cooking_color = "#FF9800"  # Ámbar
                                    cooking_bg = "#FFF8E1"
                                    cooking_status = "Sobrecocido"
                                else:
                                    cooking_color = "#2196F3"  # Azul
                                    cooking_bg = "#E3F2FD"
                                    cooking_status = "Aceptable"
                            
                            # Mostrar información en columnas
                            col1, col2 = st.columns([1, 3])
                            
                            with col1:
                                # Información básica y badge de estado
                                st.markdown(f"### {cooking_state['alimento']}")
                                
                                # Usar componentes nativos para el estado de cocción
                                state_style = f"background-color: {cooking_color}; color: white; padding: 10px; border-radius: 20px; text-align: center; font-weight: bold;"
                                st.markdown(f"<div style='{state_style}'>{cooking_state['estado_coccion']}</div>", unsafe_allow_html=True)
                                
                                st.markdown(f"**Confianza:** {int(float(cooking_state['confianza'])*100)}%")
                                
                                if cooking_state['punto_optimo']:
                                    st.success("✅ Punto óptimo de cocción")
                                else:
                                    tiempo = cooking_state['tiempo_adicional']
                                    if tiempo != "0" and tiempo != "0 minutos" and cooking_state['estado_coccion'].lower() not in ['muy hecho', 'quemado']:
                                        st.warning(f"⏱️ Necesita {tiempo} más de cocción")
                            
                            with col2:
                                # Crear pestañas para organizar la información detallada
                                cooking_tabs = st.tabs(["🍳 Detalles", "🔥 Recomendaciones"])
                                
                                with cooking_tabs[0]:
                                    st.markdown("### Características")
                                    
                                    # Crear detalles sobre el color y textura
                                    detail_cols = st.columns(2)
                                    
                                    with detail_cols[0]:
                                        st.markdown("#### Color")
                                        st.info(cooking_state['color'])
                                    
                                    with detail_cols[1]:
                                        st.markdown("#### Textura")
                                        st.info(cooking_state['textura'])
                                    
                                    # Mostrar temperatura recomendada
                                    st.markdown("#### Temperatura recomendada")
                                    st.success(cooking_state['temperatura_recomendada'])
                                
                                with cooking_tabs[1]:
                                    st.markdown("### Recomendaciones")
                                    
                                    # Mostrar recomendaciones específicas
                                    st.info(cooking_state['recomendaciones'])
                                    
                                    # Consejos adicionales basados en el tipo de alimento
                                    if "carne" in cooking_state['alimento'].lower():
                                        if cooking_state['estado_coccion'].lower() == 'crudo':
                                            st.error("⚠️ Consumir carne cruda puede causar enfermedades transmitidas por alimentos.")
                                        elif cooking_state['estado_coccion'].lower() == 'término medio':
                                            st.warning("ℹ️ La carne en término medio debe alcanzar una temperatura interna de al menos 63°C.")
                                    
                                    elif "pescado" in cooking_state['alimento'].lower():
                                        st.info("ℹ️ El pescado correctamente cocinado debe tener una temperatura interna de 63°C y separarse fácilmente en hojuelas.")
                                    
                                    elif "pollo" in cooking_state['alimento'].lower() or "ave" in cooking_state['alimento'].lower():
                                        st.warning("ℹ️ Las aves de corral deben cocinarse completamente hasta alcanzar una temperatura interna de 74°C.")
                            
                            # Botón para guardar el análisis
                            if st.button("💾 Guardar análisis de cocción", key="save_cooking_state"):
                                st.success("✅ Análisis de cocción guardado correctamente.")
                        else:
                            st.error("No se pudo determinar el estado de cocción del alimento.")
                            
                            # Datos de ejemplo como respaldo
                            st.warning("Mostrando datos de ejemplo:")
                            
                            st.markdown("""
                            ### Carne de res (ejemplo)
                            
                            **Estado de cocción:** Término medio
                            
                            **Color:** Rojo-rosado en el centro con exterior marrón
                            
                            **Textura:** Firme pero jugosa
                            
                            **Temperatura recomendada:** 63°C (145°F) en el centro
                            
                            **Recomendaciones:** Esta carne está en un punto adecuado para quienes prefieren término medio. Para término bien cocido, cocinar 2-3 minutos más por cada lado.
                            """)
